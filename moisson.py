...
#Après vérifications, le nombre d'espèces menacées recensées ne dépasse pas 88. Et quelques nombres, comme le 12 par exemple, ne sont associés à aucune espèce.
...
#J'ai laissé les traces de ma première tentative infructueuse de moissonnage. J'ai réussi à contourner mon problème avec la var "especes" susmentionnée qui renvoie à mon "range" pour compléter directement l'hyperlien initial.
...
#Donc, une fois arrivé à cette étape, je ne savais pas comment je pouvais isoler le bout d'url, unique à chaque espèce, dont j'allais me servir pour compléter mon hyperlien initial.
    
for espece in especes:
    url = "http://www3.mffp.gouv.qc.ca/faune/especes/menacees/fiche.asp?noEsp={}".format(espece)
    contenu = requests.get(url, headers=entetes)
    #Avec les deux étapes ci-dessous, je vérifie si j'ai une réponse positive de ma requête auprès du site du MFFP [statut 200], donc si les manipulations de ma première "loop" fonctionne.
